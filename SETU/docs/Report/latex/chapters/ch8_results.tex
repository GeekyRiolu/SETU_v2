...
\midrule
S0: SFT on references & Human references (Samanantar) & 10.95 & 0.389 & --- \\
\rowcolor{rowgrey}
...
English\arrowto{}Malayalam & $5\times10^{-4}$ & --- & 0.570 & $3\times10^{-4}$ & \textbf{0.863} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\endfirsthead
\multicolumn{8}{c}{\textit{Table \thetable{} --- continued from previous page}}\\
\toprule
\rowcolor{headblue}
\textbf{\#} & \textbf{Direction} & \textbf{BLEU} & \textbf{chrF} & \textbf{Teacher} & \textbf{Ratio} & \textbf{$\geq$0.80} & \textbf{Latency} \\
\midrule
...
\rowcolor{rowgrey}
\texttt{GET /languages} & --- & 23 entries: ISO code, FLORES code, script, endonym \\
\texttt{GET /models} & --- & \texttt{\{count, models: [\{pair, variant, size\_mb\}]\}} \\
\rowcolor{rowgrey}
\texttt{GET /health} & --- & \texttt{\{status, stub\}} \\
\bottomrule
\end{tabular}
\end{table}
